...
export interface SlotAlternativesInput {
  /** Item IDs the player owns. */
  bank: Set<number> | number[];
  skills: Skills;
  /** The chosen loadout we're ranking alternatives around. */
  attackType: WeaponAttackType;
  combatStyle: CombatStyle;
  weaponId: number;
  weaponName: string;
  ...
  /** Chosen item id per slot — pinned first in each slot's list. */
  chosenBySlot: Partial<Record<LoadoutSlotKey, number>>;
  /** Max alternatives per slot (including the chosen item). Default 5. */
  perSlot?: number;
}

/**
 * For a chosen loadout, rank the OTHER items the player owns that fit each
 * worn slot — best-first by the same DPS-flavoured `itemScore` the optimizer
 * uses, with the chosen item pinned to the front. This powers the "swap
 * options per slot" view (the multi-item rows in the recommended-equipment
 * bank layout): the optimizer commits to one pick, this surfaces the runners-up
 * the player could substitute.
 *
 * Degrades gracefully: with an empty bank (e.g. budget-mode from-scratch
 * builds) each slot collapses to just the chosen id.
 */
export function rankedSlotAlternatives(
  input: SlotAlternativesInput,
): Partial<Record<LoadoutSlotKey, number[]>> {
  const bankSet = input.bank instanceof Set ? input.bank : new Set<number>(input.bank);
  const bySlot = bankBySlot(bankSet, input.skills);
  const meleeStrForRanged = rangedDamageUsesMeleeStrength(input.weaponId);
  const perSlot = input.perSlot ?? 5;
  const score = (it: ItemCatalogEntry) =>
    itemScore(it, input.attackType, input.combatStyle, meleeStrForRanged);

  const out: Partial<Record<LoadoutSlotKey, number[]>> = {};
  const slots: LoadoutSlotKey[] = ["weapon", ...NON_WEAPON_SLOTS];
  for (const slot of slots) {
    // A 2H weapon leaves the shield slot empty — no alternatives to offer.
    if (slot === "shield" && input.weaponIsTwoHanded) continue;

    let pool = bySlot.get(slot) ?? [];
    if (slot === "weapon") {
      // Only weapons whose category can attack in the chosen combat style —
      // don't offer a bow as an alternative to a melee whip.
      pool = pool.filter((w) =>
        (WEAPON_STYLES[w.category] ?? []).some(
          (o) => !o.defensive && combatStyleFor(o.attackType) === input.combatStyle,
        ),
      );
    }
    if (slot === "ammo") {
      pool = pool.filter(
        (a) => checkAmmoCompatWithCategory(input.weaponName, input.weaponCategory, a.name).ok,
      );
    }
