// Non-tradeable items that aren't realistic PvE gear: PvP / Bounty Hunter
// armour and seasonal/cosmetic re-skins. These matter only for Budget mode,
// which treats owned non-tradeables as free (0 gp) so the optimizer can reach
// for genuine untradeable BIS (Dizana's quiver, Avernic treads, Barrows gloves,
// Salve amulet(ei), …). Without this filter the free-injection also surfaces
// defunct PvP gear (Morrigan's/Vesta's/…) and cosmetic clones whose catalog
// stats are duplicated or inflated (e.g. "Barrows gloves (wrapped)" carries a
// higher ranged-attack than the real Barrows gloves; "Avernic treads (max)"
// doubles the magic-strength) — so they'd wrongly out-rank the real item.
//
...
export const NON_PVE_UNTRADEABLE_IDS: ReadonlySet<number> = new Set(
  ITEM_CATALOG.filter((it) => NON_PVE_RE.test(it.name)).map((it) => it.id),
);
